...
public final class PluginUpdater {
    ...
    private volatile String lastRestartNoticeVersion;
    ...
    private void announceRestartRequired(UpdateInfo info) {
        if (info == null
            || !info.enabled()
        ...
        synchronized (this) {
            if (info.latestVersion().equals(lastRestartNoticeVersion)) {
                return;
            }
            lastRestartNoticeVersion = info.latestVersion();
        }
