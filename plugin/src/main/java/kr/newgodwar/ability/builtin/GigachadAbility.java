...
final class GigachadAbility extends BaseAbility {
    ...
    protected void onStaffLeft(AbilityPlayerContext context, Player player, PlayerInteractEvent event) {
        ...
        if (useNormal(context, player)) {
            ...
            effect(context, player, "RESISTANCE", "DAMAGE_RESISTANCE", 10, 0);
            effect(context, player, PotionEffectType.ABSORPTION, 10, 0);
        }
    }
    ...
    protected void onStaffRight(AbilityPlayerContext context, Player player, PlayerInteractEvent event) {
        if (useAdvanced(context, player)) {
            effect(context, player, PotionEffectType.SPEED, 11, 1);
            effect(context, player, "STRENGTH", "INCREASE_DAMAGE", 11, 0);
            effect(context, player, "RESISTANCE", "DAMAGE_RESISTANCE", 11, 0);
        }
    }
    ...
    public void onGenericDamage(AbilityPlayerContext context, EntityDamageEvent event) {
        ...
        if (player.getHealth() - event.getFinalDamage() <= 6.0D && rollChance(1, 4)) {
            effect(context, player, PotionEffectType.ABSORPTION, 10, 1);
