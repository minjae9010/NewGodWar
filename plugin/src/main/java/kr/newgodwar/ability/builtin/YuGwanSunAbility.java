...
final class YuGwanSunAbility extends BaseAbility {
    ...
    protected void onStaffLeft(AbilityPlayerContext context, Player player, PlayerInteractEvent event) {
        ...
        for (Player ally : allies) {
            effect(context, ally, PotionEffectType.SPEED, 12, 1);
            effect(context, ally, "STRENGTH", "INCREASE_DAMAGE", 10, 0);
            effect(context, ally, "RESISTANCE", "DAMAGE_RESISTANCE", 9, 0);
        }
        for (Player enemy : enemies) {
            effect(context, enemy, PotionEffectType.WEAKNESS, 10, 0);
            effect(context, enemy, "SLOWNESS", "SLOW", 10, 1);
        }
    ...
    protected void onStaffRight(AbilityPlayerContext context, Player player, PlayerInteractEvent event) {
        ...
        for (Player ally : allies) {
            effect(context, ally, "ABSORPTION", "ABSORPTION", 12, 1);
            effect(context, ally, PotionEffectType.REGENERATION, 10, 1);
