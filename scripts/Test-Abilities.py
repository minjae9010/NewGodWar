"""Run ability regression on a cached Paper server without downloading dependencies.

Build first: gradlew build
Run: python scripts/Test-Abilities.py [26.3]
...
probe = root / "plugin/build/ability-regression/AbilityRegressionProbe.jar"
...
target = Path(tempfile.mkdtemp(prefix="ability-variety-" + version + "-", dir=root / ".paper-smoke"))
...
shutil.copy2(probe, target / "plugins/AbilityRegressionProbe.jar")
...
try:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            continue
        lines.append(line)
        if any(word in line for word in ("ABILITY REGRESSION", "PASS ", "ERROR", "Exception", "Done (")):
            print(line.rstrip(), flush=True)
        if "ABILITY REGRESSION PASS" in line:
            passed = True
            break
        if "ABILITY REGRESSION FAILED" in line:
            break
finally:
    ...
    (target / "ability-regression.log").write_text("".join(lines), encoding="utf-8")
if not passed:
    failure = next((i for i, line in enumerate(lines) if "ABILITY REGRESSION FAILED" in line), None)
    print("".join(lines[failure:failure + 60] if failure is not None else lines[-90:]))
    raise SystemExit("Ability regression failed; logs: " + str(target))
print("Ability regression passed on Paper " + version + "; logs: " + str(target), flush=True)
