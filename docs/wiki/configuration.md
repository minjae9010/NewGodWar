...
1. `/gw settings`를 열고 상단의 **게임 시작 준비 체크리스트**에서 팀 배정 인원, 활성 팀, 스폰·심장 위치, 게임 월드를 확인합니다.
2. 월드 메뉴에서 게임 월드를 확인하고, 팀 메뉴에서 각 팀의 스폰과 다이아 심장을 등록합니다. 위치가 등록되어 있어도 해당 월드가 로드되지 않으면 준비되지 않은 것으로 표시됩니다.
3. 게임 진행 메뉴에서 팀 자동 배정과 최소 인원을 확인한 뒤 게임을 시작합니다. 실제 시작 시 최종 검사와 필요한 월드 백업을 수행합니다. 애드온 게임 모드는 자체 시작 조건을 사용합니다.
4. 게임 종료 버튼을 누르면 확인 화면이 열립니다. **게임 종료 확정**을 좌클릭해야 종료되며, 취소·뒤로가기·창 닫기는 게임을 유지합니다.

GUI에서 바꾼 설정은 자동 저장됩니다. **설정 다시 불러오기**는 파일을 직접 수정했을 때 사용합니다.


### 코어 파괴와 시작 검사

심장은 참가 중인 생존 팀 플레이어만 파괴할 수 있으며, 자기 팀 심장은 파괴할 수 없습니다. 같은 다이아몬드 블록을 두 팀의 심장으로 등록할 수 없습니다. 기존 파일에 중복 위치가 있으면 해당 팀 이름과 함께 시작 검사에서 안내합니다.

하나의 폭발로 여러 심장이 파괴될 때는 모든 탈락을 반영한 후 승패를 한 번만 판정합니다. 생존 팀이 없으면 무승부로 종료하며 `messages.draw`로 문구를 바꿀 수 있습니다. 준비 중 퇴장·관전 전환 등으로 최소 인원이 부족해지면 카운트다운 종료 시 게임 시작이 취소됩니다.

능력별 동작·연출과 중복 방지 규칙은 [능력 이펙트 안내](https://github.com/minjae9010/NewGodWar/wiki/ability-effects)에서 확인할 수 있습니다.
