...
저장소의 `examples/addon`은 능력과 게임 기능을 모두 포함합니다. 아래 JAR 경로는 `0.3.5` 기준입니다. 다른 버전을 빌드하면 저장소 루트 `build.gradle`의 `version`에 맞춰 경로를 바꾸세요.

```powershell
.\gradlew.bat build :example-addon:build
```

- 본체: `build/libs/NewGodWar-0.3.5.jar`
- 예제: `examples/addon/build/libs/NewGodWar-ExampleAddon-0.3.5.jar`
...
dependencies {
    compileOnly files('libs/NewGodWar-0.3.5.jar')
